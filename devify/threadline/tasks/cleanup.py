...
from threadline.utils.task_tracer import TaskTracer, get_current_task_tracer
...
class EmailCleanupManager:
    ...
    def __init__(self):
        ...
        self.tracer = get_current_task_tracer() or TaskTracer("HARAKA_CLEANUP")
    ...
    def cleanup_haraka_files(self) -> Dict:
        ...
        self.tracer.append_task(
            "CLEANUP_START",
            "Haraka cleanup started",
            {"directory": "all", "stats": stats.copy()},
        )
        context = self.tracer.context_summary(
            {"cleanup_type": "HARAKA_CLEANUP"}
        )

        try:
            logger.info(f"{context} Starting Haraka email files cleanup")

            # Clean up inbox directory
            self.tracer.append_task(
                "CLEANUP_PHASE",
                "Cleaning inbox directory",
                {"directory": "inbox"},
            )
            logger.info(f"{context} Cleaning inbox directory")
            ...
            logger.info(f"{context} Cleaning processed directory")
            ...
            stats["total_files_removed"] += processed_stats["files_cleaned"]
            ...
            self.tracer.append_task(
                "CLEANUP_PHASE",
                "Cleaning failed directory",
                {"directory": "failed"},
            )
            logger.info(f"{context} Cleaning failed directory")
            ...
            details = self._build_details_dict(
                stats, inbox_stats, processed_stats, failed_stats
            )
            details["completed_at"] = timezone.now().isoformat()
            self.tracer.append_task(
                "CLEANUP_COMPLETE",
                "Haraka cleanup finished",
                details.copy(),
            )
            self.tracer.complete_task(details)

            logger.info(f"{context} Haraka cleanup completed: {stats}")
            return stats

        except Exception as exc:
            logger.error(f"{context} Haraka cleanup failed: {exc}")
            stats["errors"] += 1
    ...
    def get_directory_stats(self, directory_path: str) -> Dict:
        ...
        try:
            ...
            logger.error(
                f"Error getting directory stats for " f"{directory_path}: {e}"
            )
            return stats
    ...
    def _build_details_dict(
        ...
        return {
            "cleanup_type": "HARAKA_CLEANUP",
            "total_files_removed": total_stats.get("total_files_removed", 0),
    ...
    def _cleanup_directory(
        self, directory_path: str, timeout_seconds: int, directory_type: str
    ) -> Dict:
        """
        ...
        """
        stats = {"files_cleaned": 0, "size_freed": 0, "errors": 0}

        if not os.path.exists(directory_path):
            logger.warning(
                f"{self.tracer.context_summary({'cleanup_type': 'HARAKA_CLEANUP', 'directory': directory_type})} "
        ...
        try:
            ...
            if not files:
                logger.debug(
                    f"{self.tracer.context_summary({'cleanup_type': 'HARAKA_CLEANUP', 'directory': directory_type})} "
                    f"No files found in {directory_type} directory"
                )
                return stats
            ...
            logger.info(
                f"{self.tracer.context_summary({'cleanup_type': 'HARAKA_CLEANUP', 'directory': directory_type})} "
                f"Found {len(meta_files)} meta files in {directory_type} directory"
            )
            ...
            for meta_file in meta_files:
                try:
                    uuid = meta_file.replace(".meta", "")
                    meta_path = os.path.join(directory_path, meta_file)
                    eml_path = os.path.join(directory_path, f"{uuid}.eml")

                    # Check if both files exist
                    if not os.path.exists(eml_path):
                        logger.warning(
                            f"{self.tracer.context_summary({'cleanup_type': 'HARAKA_CLEANUP', 'directory': directory_type})} "
                            f"Missing .eml file: {uuid} in {directory_type}"
                        )
                        continue
                    ...
                    if file_time < cutoff_time:
                        ...
                        logger.debug(
                            f"{self.tracer.context_summary({'cleanup_type': 'HARAKA_CLEANUP', 'directory': directory_type})} "
                            f"Cleaned up {uuid} from {directory_type} directory"
                        )

                except Exception as e:
                    logger.error(
                        f"{self.tracer.context_summary({'cleanup_type': 'HARAKA_CLEANUP', 'directory': directory_type})} "
                        f"Error cleaning up {meta_file} in {directory_type}: {e}"
                    )
                    stats["errors"] += 1
                    continue
            ...
            logger.info(
                f"{self.tracer.context_summary({'cleanup_type': 'HARAKA_CLEANUP', 'directory': directory_type})} "
                f"Cleaned {stats['files_cleaned']} files from {directory_type} directory"
            )
            return stats
            ...
            logger.error(
                f"{self.tracer.context_summary({'cleanup_type': 'HARAKA_CLEANUP', 'directory': directory_type})} "
                f"Error cleaning {directory_type} directory: {e}"
            )
            stats["errors"] += 1
            return stats
...
class EmailTaskCleanupManager:
    ...
    def __init__(self):
        """Initialize the cleanup manager with configuration."""
        self.config = settings.EMAIL_CLEANUP_CONFIG
        self.tracer = get_current_task_tracer() or TaskTracer("TASK_CLEANUP")
    ...
    def cleanup_email_tasks(self) -> Dict:
        ...
        Returns:
            Dict containing cleanup statistics
        """
        stats = {"tasks_cleaned": 0, "errors": 0}

        # Create task record
        self.tracer.create_task(
            {"cleanup_type": "TASK_CLEANUP", "tasks_cleaned": 0, "errors": 0}
        )
        self.tracer.append_task(
            "CLEANUP_START",
            "EmailTask cleanup started",
            {"cleanup_type": "TASK_CLEANUP"},
        )
        context = self.tracer.context_summary({"cleanup_type": "TASK_CLEANUP"})

        try:
            retention_days = self.config["email_task_retention_days"]
            logger.info(
                f"{context} Starting EmailTask cleanup (retention: {retention_days} days)"
            )
            ...
            # Get old tasks
            old_tasks = EmailTask.objects.filter(created_at__lt=cutoff_date)

            task_count = old_tasks.count()
            logger.info(
                f"{context} Found {task_count} old EmailTask records to clean up"
            )
            ...
            if task_count > 0:
                ...
                while True:
                    ...
                    self.tracer.append_task(
                        "CLEANUP_BATCH",
                        "Deleted a batch of EmailTask records",
                        {
                            "deleted": deleted,
                    ...
                    logger.debug(
                        f"{context} Deleted batch of {deleted} EmailTask records"
                    )

                stats["tasks_cleaned"] = deleted_count
                logger.info(
                    f"{context} Cleaned up {deleted_count} EmailTask records"
                )
            ...
            logger.error(f"{context} EmailTask cleanup failed: {e}")
            stats["errors"] += 1
            details = {
                "cleanup_type": "TASK_CLEANUP",
                "tasks_cleaned": stats["tasks_cleaned"],
...
class ShareLinkCleanupManager:
    ...
    def __init__(self):
        ...
        self.tracer = get_current_task_tracer() or TaskTracer(
            "SHARE_LINK_CLEANUP"
        )
    ...
    def cleanup_expired_share_links(self) -> Dict:
        ...
        Returns:
            Dict containing cleanup statistics
        """
        stats = {"links_considered": 0, "links_deactivated": 0, "errors": 0}
        ...
        expired_queryset = ThreadlineShareLink.objects.filter(**base_filters)
        ...
        if stats["links_considered"] == 0:
            ...
            self.tracer.append_task(
                "CLEANUP_COMPLETE",
                "Share link cleanup finished",
                task_details.copy(),
            )
            self.tracer.complete_task(task_details)
            logger.info(
                f"{context} Share link cleanup: no expired links found."
            )
            return stats
        ...
        try:
            while True:
                ...
                self.tracer.append_task(
                    "CLEANUP_BATCH",
                    "Deactivated expired share links",
                    task_details.copy(),
                )
            ...
            logger.info(
                f"{context} Share link cleanup completed. Deactivated {stats['links_deactivated']} expired links."
            )
            return stats
            ...
            self.tracer.append_task(
                "CLEANUP_ERROR",
                f"Share link cleanup failed: {exc}",
                task_details.copy(),
            )
            self.tracer.fail_task(task_details, str(exc))
            logger.error(f"{context} Share link cleanup failed: {exc}")
            return stats
