...
This command provides manual cleanup of email files and related business
resources with various options for configuration and monitoring.
...
class Command(BaseCommand):
    ...
    help = "Clean up email files and related business resources"
    ...
    def _show_configuration(self, options):
        ...
        if options["inbox_only"]:
            ...
            self.stdout.write("Mode: All directories")

        # Show timeout settings
        config = self._get_cleanup_config()
        self.stdout.write(
            f"Inbox timeout: " f"{config['inbox_timeout_hours']} hours"
        )
        self.stdout.write(
            f"Processed timeout: "
        ...
        self.stdout.write(
            f"Failed timeout: " f"{config['failed_timeout_minutes']} minutes"
        )
        # Show custom overrides
        if options["inbox_timeout"]:
            self.stdout.write(
                f"Inbox timeout override: " f"{options['inbox_timeout']} hours"
            )
        if options["processed_timeout"]:
            self.stdout.write(
                f"Processed timeout override: "
                f"{options['processed_timeout']} minutes"
            )
        if options["failed_timeout"]:
            self.stdout.write(
                f"Failed timeout override: "
    ...
    def _show_statistics(self, options):
        ...
        self.stdout.write(self.style.SUCCESS("Email File Cleanup Statistics"))
        self.stdout.write("=" * 50)
        ...
        try:
            ...
            for directory in ["inbox", "processed", "failed"]:
                ...
                self.stdout.write(
                    f"  Meta files: " f"{dir_stats['meta_files']}"
                )
                self.stdout.write(f"  EML files: " f"{dir_stats['eml_files']}")
                self.stdout.write(
                    f"  Orphaned files: " f"{dir_stats['orphaned_files']}"
                )
                self.stdout.write("")

            self.stdout.write(f"Total files: {stats['total_files']}")
            self.stdout.write(
                f"Total size: " f"{self._format_size(stats['total_size'])}"
            )
            self.stdout.write("")
    ...
    def _perform_cleanup(self, options):
        ...
        self.stdout.write(self.style.SUCCESS("Starting email file cleanup..."))
    ...
    def _cleanup_email_files(self, options):
        ...
        try:
            ...
            self.stdout.write("-" * 30)
            self.stdout.write(
                f"Inbox cleaned: " f"{result.get('inbox_cleaned', 0)}"
            )
            self.stdout.write(
                f"Processed cleaned: " f"{result.get('processed_cleaned', 0)}"
            )
            self.stdout.write(
                f"Failed cleaned: " f"{result.get('failed_cleaned', 0)}"
            )
            self.stdout.write(
                f"Total files removed: "
                f"{result.get('total_files_removed', 0)}"
            )
            size_freed = self._format_size(result.get("total_size_freed", 0))
