...
class EmailConfigManagerTest(TestCase):
    ...
    def test_detect_email_source_file(self):
        ...
        Settings.objects.create(
            user=self.user,
            key="email_config",
            value={"file_config": {"email_directory": "/path/to/emails"}},
            is_active=True,
        )
...
class EmailSaveServiceTest(TestCase):
    ...
    def test_save_email_success(self):
        ...
        self.assertEqual(email_msg.raw_message_id, "")
        self.assertEqual(email_msg.in_reply_to, "")
        self.assertEqual(email_msg.references, [])
    ...
    def test_find_user_by_recipient_no_match(self):
        ...
        email_data = {"recipients": ["nonexistent@example.com"]}
    ...
    def test_find_user_by_recipient_exception(self):
        ...
        with patch(
            "threadline.utils.email.service.User.objects.get"
        ) as mock_get:
            mock_get.side_effect = Exception("Database error")
    ...
    def test_process_attachments(self):
        ...
        with (
            patch(
                "threadline.utils.email.service.os.makedirs"
            ) as mock_makedirs,
            patch("threadline.utils.email.service.shutil.copy2") as mock_copy,
            patch(
                "threadline.utils.email.service.os.path.exists"
            ) as mock_exists,
            patch.object(self.service, "_compute_file_md5") as mock_md5,
            patch(
                "threadline.utils.email.service.EmailAttachment.objects.create"
            ) as mock_create,
        ):
            mock_exists.return_value = True
            mock_attachment = Mock()
            mock_create.return_value = mock_attachment
            mock_md5.return_value = "0123456789abcdef0123456789abcdef"
    ...
    def test_process_attachments_reuses_cached_image_content(self):
        """Test cross-email attachment reuse without mocks."""
        with (
            tempfile.TemporaryDirectory() as attachment_dir,
            tempfile.TemporaryDirectory() as source_dir,
            override_settings(EMAIL_ATTACHMENT_DIR=attachment_dir),
        ):
            source_path = os.path.join(source_dir, "diagram.png")
            image_bytes = b"cached-image-content"
            with open(source_path, "wb") as file_handle:
                file_handle.write(image_bytes)

            first_email = self.service.save_email(
                self.user.id,
                {
                    "subject": "First email",
                    "sender": "sender@example.com",
            ...
            first_attachment = first_email.attachments.get()
            first_attachment.ocr_content = "recognized text"
            first_attachment.llm_content = "vision summary"
            first_attachment.save(update_fields=["ocr_content", "llm_content"])

            second_email = self.service.save_email(
                self.user.id,
                {
            ...
            second_attachment = second_email.attachments.get()

            self.assertEqual(
                first_attachment.content_md5, second_attachment.content_md5
            )
            self.assertEqual(second_attachment.ocr_content, "recognized text")
            self.assertEqual(second_attachment.llm_content, "vision summary")
            self.assertTrue(os.path.exists(second_attachment.file_path))
            self.assertEqual(
                os.stat(first_attachment.file_path).st_ino,
                os.stat(second_attachment.file_path).st_ino,
            )
    ...
    def test_process_attachments_exception(self):
        ...
        attachments = [{"filename": "test.pdf", "file_path": "/tmp/test.pdf"}]
