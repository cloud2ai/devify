...
class TaskLockTest(TestCase):
    ...
    def test_acquire_task_lock_exception(self):
        ...
        with patch(
            "agentcore_task.adapters.django.services.lock.cache.add"
        ) as mock_add:
    ...
    def test_release_task_lock_exception(self):
        ...
        with patch(
            "agentcore_task.adapters.django.services.lock.cache.delete"
        ) as mock_delete:
    ...
    def test_is_task_locked_exception(self):
        ...
        with patch(
            "agentcore_task.adapters.django.services.lock.cache.get"
        ) as mock_get:
    ...
    def test_force_release_all_locks_exception(self):
        ...
        with patch(
            "threadline.utils.task_lock.cache.delete_many"
        ) as mock_delete_many:
    ...
    def test_prevent_duplicate_task_decorator_already_running(self):
        """Test prevent_duplicate_task when the task is already running."""
        # Acquire lock first
        acquire_task_lock("test_task", timeout=60)
    ...
    def test_lock_timeout_expiration(self):
        ...
        cache.delete("agentcore_task_task_lock:test_task")
    ...
    def test_lock_key_format(self):
        ...
        with patch(
            ...
            mock_add.assert_called_once_with(
                "agentcore_task_task_lock:test_task", "locked", timeout=60
            )
