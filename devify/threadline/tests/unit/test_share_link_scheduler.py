...
class ShareLinkSchedulerTest(TestCase):
    ...
    def test_schedule_share_link_cleanup_success(
        self, mock_manager, mock_is_locked, mock_acquire, mock_release
    ):
        ...
        manager_instance.cleanup_expired_share_links.return_value = (
            cleanup_stats
        )
        ...
        mock_acquire.assert_called_once_with("share_link_cleanup", 1800)
    ...
    def test_schedule_share_link_cleanup_failure(
        ...
        manager_instance = MagicMock()
        manager_instance.cleanup_expired_share_links.side_effect = (
            RuntimeError("boom")
        )
        mock_manager.return_value = manager_instance
