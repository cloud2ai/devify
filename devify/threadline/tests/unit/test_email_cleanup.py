...
class EmailCleanupManagerTest(TestCase):
    ...
    def test_cleanup_haraka_files_success(self, mock_tracer, mock_settings):
        ...
        self._create_test_email_files(self.inbox_dir, "test1", age_minutes=70)
        self._create_test_email_files(
            self.processed_dir, "test2", age_minutes=15
        )
    ...
    def test_cleanup_haraka_files_partial_cleanup(self, mock_settings):
        ...
        self._create_test_email_files(self.inbox_dir, "old", age_minutes=70)
        self._create_test_email_files(self.inbox_dir, "new", age_minutes=5)
    ...
    def test_cleanup_haraka_files_with_error(
        self, mock_join, mock_tracer, mock_settings
    ):
    ...
    def _create_test_email_files(self, directory, uuid, age_minutes=0):
        """
...
class EmailTaskCleanupManagerTest(TestCase):
    ...
    def test_cleanup_email_tasks_success(self, mock_tracer, mock_settings):
        ...
        mock_settings.EMAIL_CLEANUP_CONFIG = {"email_task_retention_days": 3}
    ...
    def test_cleanup_email_tasks_with_error(
        self, mock_logger, mock_tracer, mock_settings
    ):
    ...
    def test_cleanup_email_tasks_different_retention(self, mock_settings):
        ...
        mock_settings.EMAIL_CLEANUP_CONFIG = {"email_task_retention_days": 7}
...
class ShareLinkCleanupManagerTest(TestCase):
    ...
    @override_settings(
        SHARE_LINK_CLEANUP_CONFIG={"grace_period_minutes": 0, "batch_size": 10}
    )
    ...
    @override_settings(
        SHARE_LINK_CLEANUP_CONFIG={"grace_period_minutes": 0, "batch_size": 1}
    )
    def test_cleanup_processes_in_batches(self):
        ...
        links = ThreadlineShareLinkFactory.create_batch(
            3, expires_at=timezone.now() - timedelta(hours=3)
        )
