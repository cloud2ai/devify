...
class EmailFetchTasksTest(TestCase):
    ...
    def test_imap_email_fetch_success(
        self, mock_fetch, mock_get_config, mock_filter, mock_create
    ):
    ...
    @patch("threadline.tasks.email_fetch.User.objects.filter")
    @patch("threadline.tasks.email_fetch.EmailConfigManager.get_email_config")
    def test_imap_email_fetch_no_config(
        self, mock_get_config, mock_filter, mock_create
    ):
    ...
    def test_haraka_email_fetch_success(
        self, mock_processor_class, mock_save_service_class
    ):
    ...
    @patch("threadline.tasks.email_fetch.EmailProcessor")
    @patch("threadline.tasks.email_fetch.EmailSaveService")
    def test_fetch_user_imap_emails_success(
        self, mock_save_service_class, mock_processor_class, mock_get_config
    ):
        ...
        mock_processor_class.return_value = mock_processor
        mock_processor.process_emails.return_value = [
            {"subject": "Test Email", "message_id": "test-123"}
        ]
    ...
    def test_imap_email_fetch_success(
        self, mock_fetch, mock_get_config, mock_users, mock_create
    ):
    ...
    def test_fetch_user_imap_emails_success(
        ...
        # Mock email data
        mock_email_data = {"subject": "Test Email", "message_id": "test-123"}
        mock_processor.process_emails.return_value = [mock_email_data]
        ...
        mock_acquire.assert_called_once_with(
            f"fetch_user_imap_emails_{self.user.id}",
            timeout=600,
        )
    ...
    @patch("threadline.tasks.email_fetch.haraka_email_fetch.delay")
    @patch("agentcore_task.adapters.django.services.lock.release_task_lock")
    def test_schedule_email_fetch_exception_handling(
        self,
        mock_release,
        mock_haraka_delay,
        mock_imap_delay,
