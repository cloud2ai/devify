...
class TestAuthentication:
    ...
    def test_threadlines_endpoints_require_auth(self, api_client):
        ...
        url = reverse(
            "threadlines-detail",
            kwargs={"uuid": "00000000-0000-0000-0000-000000000001"},
        )
        response = api_client.get(url)
        assert response.status_code == status.HTTP_401_UNAUTHORIZED
    ...
    def test_expired_token_denies_access(self, api_client, test_user):
        ...
        access_token.set_exp(
            from_time=access_token.current_time - timedelta(hours=1)
        )
...
class TestUserDataIsolation:
    ...
    def test_threadlines_user_isolation(
        self,
        authenticated_api_client,
        test_user,
        authenticated_api_client_2,
        ...
        message1 = EmailMessageFactory(
            user=test_user, subject="User 1 Message"
        )
        message2 = EmailMessageFactory(
            user=test_user_2, subject="User 2 Message"
        )
        ...
        url = reverse("threadlines-detail", kwargs={"uuid": message2.uuid})
        response = authenticated_api_client.get(url)
        assert response.status_code == status.HTTP_404_NOT_FOUND

        # User 2 cannot access User 1's message details
        url = reverse("threadlines-detail", kwargs={"uuid": message1.uuid})
        response = authenticated_api_client_2.get(url)
        assert response.status_code == status.HTTP_404_NOT_FOUND

    def test_cross_user_data_modification_prevention(
        self,
        authenticated_api_client,
        test_user,
        authenticated_api_client_2,
        ...
        response = authenticated_api_client.put(
            url, {"value": {"hacked": "value"}}, format="json"
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND

        # User 2 cannot update User 1's setting
        url = reverse("settings-detail", kwargs={"pk": setting1.id})
        response = authenticated_api_client_2.put(
            url, {"value": {"hacked": "value"}}, format="json"
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND
        ...
        url = reverse("threadlines-detail", kwargs={"uuid": message2.uuid})
        response = authenticated_api_client.put(
            url, {"subject": "Hacked Subject"}, format="json"
        )
        assert response.status_code == status.HTTP_404_NOT_FOUND
    ...
    def test_cross_user_data_deletion_prevention(
        self,
        authenticated_api_client,
        test_user,
        authenticated_api_client_2,
...
class TestPermissionEdgeCases:
    ...
    def test_nonexistent_user_handling(
