...
class TestEmailTodoAPI:
    ...
    def test_list_todos_authenticated_with_data(
        ...
        assert response.data["data"]["list"][0]["id"] == test_email_todo.id
        assert (
    ...
    def test_list_todos_filter_completed(
        ...
        assert response.status_code == status.HTTP_200_OK
        assert len(response.data["data"]["list"]) == 1
        assert response.data["data"]["list"][0]["is_completed"] is True
    ...
    def test_list_todos_filter_email_message(
        ...
        todo1 = EmailTodoFactory(user=test_user, email_message=email_message)
    ...
    def test_list_todos_search(self, authenticated_api_client, test_user):
        """
    ...
    def test_list_todos_ordering(self, authenticated_api_client, test_user):
        """
        Test ordering TODOs
        """
        EmailTodo.objects.all().delete()
    ...
    def test_list_todos_pagination(self, authenticated_api_client, test_user):
        """
    ...
    def test_create_todo_unauthenticated(self, api_client):
        ...
        """
        url = reverse("todos-list")
        data = {"content": "Test TODO", "priority": "high"}
    ...
    def test_create_todo_invalid_priority(
        ...
        """
        url = reverse("todos-list")
        data = {"content": "Test TODO", "priority": "invalid_priority"}
        response = authenticated_api_client.post(url, data, format="json")
    ...
    def test_update_todo_patch(
        ...
        """
        url = reverse("todos-detail", kwargs={"pk": test_email_todo.id})
        data = {"is_completed": True, "priority": "high"}
    ...
    def test_update_todo_other_user(self, authenticated_api_client, test_user):
        """
    ...
    def test_delete_todo_other_user(self, authenticated_api_client, test_user):
        """
    ...
    def test_delete_todo_not_found(self, authenticated_api_client, test_user):
        """
...
class TestEmailTodoStatsAPI:
    ...
    def test_stats_authenticated_with_data(
        ...
        assert response.data["data"]["by_priority"]["high"] == 2
        assert response.data["data"]["by_priority"]["medium"] == 1
        assert response.data["data"]["by_priority"]["low"] == 1

    def test_stats_user_isolation(self, authenticated_api_client, test_user):
        """
...
class TestEmailTodoBatchAPI:
    ...
    def test_batch_complete_success(self, authenticated_api_client, test_user):
        """
