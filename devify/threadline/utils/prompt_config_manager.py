...
class PromptConfigManager:
    ...
    def _load_scene_prompts(self, scene: str) -> Optional[Dict[str, Any]]:
        """
        ...
        if scene in self.scene_prompts_cache:
            logger.debug(f"Using cached scene prompts for scene: {scene}")
        ...
        if not prompt_file:
            logger.warning(f"No prompt_file specified for scene '{scene}'")
            return None
    ...
    def _render_prompt(
        self, prompt_text: str, shared_snippets: Dict[str, str]
    ) -> str:
        """
    ...
    def _merge_prompts(
        self, scene_prompts: Optional[Dict[str, Any]], language: str
    ) -> Dict[str, Any]:
        """
        ...
        default_shared_lang = (
            ...
            if settings.DEFAULT_LANGUAGE in shared_snippets_all
                ...
                list(shared_snippets_all.keys())[0]
        ...
        logger.info(
            f"Using language for prompts: {lang_display} " f"(code={language})"
        )
    ...
    def get_scene_config(self, scene: str, language: str) -> Dict[str, Any]:
        ...
        if lang not in scene_info:
            raise KeyError(f"Language '{lang}' not found for scene '{scene}'")
    ...
    def get_prompt_config(self, scene: str, language: str) -> Dict[str, Any]:
        """
        ...
        logger.info(
            ...
            f"has_email_content_prompt="
            f"{'email_content_prompt' in merged_prompts}, "
            f"has_image_intent_prompt="
            f"{'image_intent_prompt' in merged_prompts}"
        )
    ...
    def _find_language_config(self, language: str) -> Optional[Dict[str, Any]]:
        ...
        for lang in languages:
            ...
            if code and (
                code.startswith(language_lower[:2])
                or language_lower.startswith(code[:2])
            ):
                logger.debug(
    ...
    def _normalize_language(self, language: str) -> str:
        ...
        if lang_config:
            ...
            logger.debug(f"_normalize_language: {language} -> {normalized}")
            return normalized
    ...
    def load_prompt_config(self, user) -> Dict[str, Any]:
        ...
        try:
            ...
            has_full_prompts = any(
                key in prompt_config_raw for key in legacy_prompt_fields
            )
            ...
            if has_full_prompts:
                ...
                # generation
                custom_prompts = [
                    k for k in legacy_prompt_fields if k in prompt_config_raw
                ]
                logger.info(
                ...
                if (
                    "language" not in prompt_config_raw
                    or "scene" not in prompt_config_raw
                ):
                    prompt_config_raw = {
                        **prompt_config_raw,
