...
scene_prompt_config:
  chat:
    zh-CN:
      ...
      ocr_prompt: "将清洗后的OCR内容结合对话上下文和图片文件名进行推理，说明用户为什么要在当前对话中插入这张图片，这张图片想支持、说明、证明或补充什么信息。输出时尽可能保留所有与这个“为什么”直接相关的重要识别内容，包括错误信息、状态、数值、时间、标签、标题、按钮、页面名称、警告和字段值；仅删除明显无关的噪声、装饰元素和通用界面杂项。如上下文不足，则谨慎说明可能目的并更完整保留OCR内容；不确定部分标记为[不清楚]；不要添加OCR或上下文之外的解释、总结或结论。"
    ...
    en-US:
      ...
      ocr_prompt: "Interpret the cleaned OCR content together with the conversation context and image filename. Explain why the user likely inserted this image into the conversation and what the image is trying to support, prove, illustrate, report, or clarify. Preserve as much OCR content as possible when it helps explain that purpose, including errors, statuses, values, timestamps, labels, titles, buttons, warnings, and field values. Remove only clearly irrelevant noise, decorative elements, repeated boilerplate, or generic UI chrome. If the conversation context is weak or ambiguous, provide a cautious explanation of the likely purpose and preserve the OCR content more completely. Mark uncertain parts as [unclear], and do not add conclusions beyond the OCR and context."
  ...
  product_issue:
    zh-CN:
      ...
      ocr_prompt: "将清洗后的OCR内容结合对话上下文和图片文件名进行推理，说明用户为什么要在当前对话中插入这张图片，这张图片想支持、说明、证明、报告或补充什么信息。输出时尽可能保留所有与这个“为什么”直接相关的重要识别内容，包括错误信息、状态、数值、时间、标签、标题、按钮、页面名称、警告和字段值；仅删除明显无关的噪声、装饰元素、重复内容、通用界面杂项和模板化内容。如上下文不足，则谨慎说明可能目的并更完整保留OCR内容；不确定部分标记为[不清楚]；不要添加OCR或上下文之外的解释、总结或结论。"
    ...
    en-US:
      ...
      ocr_prompt: "Interpret the cleaned OCR content together with the conversation context and image filename. Explain why the user likely inserted this image into the conversation and what the image is trying to support, prove, illustrate, report, or clarify. Preserve as much OCR content as possible when it helps explain that purpose, including errors, statuses, values, timestamps, labels, titles, buttons, warnings, and field values. Remove only clearly irrelevant noise, decorative elements, repeated boilerplate, generic UI chrome, and template-like fragments. If the conversation context is weak or ambiguous, provide a cautious explanation of the likely purpose and preserve the OCR content more completely. Mark uncertain parts as [unclear], and do not add conclusions beyond the OCR and context."
